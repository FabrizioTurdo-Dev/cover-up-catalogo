...
import { formatPrice, getPriceByQuantity } from "../data/store";
import { supabase } from "../config/supabase"; // Ajustá la ruta según tus carpetas
const SELLER_PHONE = "1154922800";
...
export default function CatalogoCoverUp() {
  const { products, addOrder, priceConfig } = useApp();
    ...
    async function fetchCatalogo() {
      try {
        const { data, error } = await supabase
          .from('cover_up') // Asegúrate de que este sea el nombre exacto de tu tabla
          .select('*')
          .order('id', { ascending: true });

        if (error) throw error;

        // Transformamos los datos de la DB al formato que usa tu frontend
        const productosFormateados = data.map(dbItem => ({
          id: dbItem.id,
          code: dbItem.codigos,
          name: dbItem.articulo, // Acá conectamos el "articulo" de Supabase con el "name" de React
          cat: dbItem.categoria ? dbItem.categoria.toLowerCase() : 'organizadores',
          details: dbItem.medidas ? `Medida: ${dbItem.medidas}` : '',
          active: true, // Forzamos a true para que se muestren
          emoji: '📦', // Placeholder visual
          // Si tienes los precios en BD, pásalos aquí:
        ...
        setRealProducts(productosFormateados || []);
      } catch (err) {
...
const categories = useMemo(() => {
    const cats = new Set();
    // AHORA USA realProducts EN VEZ DE products
    realProducts.forEach(p => cats.add(p.cat));
    return [...cats].sort();
  }, [realProducts]);
...
const filtered = useMemo(() => realProducts.filter(p => {
  ...
  }), [realProducts, catFilter, searchTerm]);
